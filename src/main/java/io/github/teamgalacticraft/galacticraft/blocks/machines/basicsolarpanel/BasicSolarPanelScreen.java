 ...
 */
public class BasicSolarPanelScreen extends ContainerScreen {
    ...
    public BasicSolarPanelScreen(int syncId, BlockPos blockPos, PlayerEntity playerEntity) {
        super(new BasicSolarPanelContainer(syncId, blockPos, playerEntity), playerEntity.inventory, new TranslatableTextComponent("ui.galacticraft-rewoven.basic_solar_panel.name"));
    ...
    public void render(int mouseX, int mouseY, float v) {
        ...
        DrawableUtils.drawCenteredString(this.minecraft.textRenderer, new TranslatableTextComponent("block.galacticraft-rewoven.basic_solar_panel_block").getText(), (this.width / 2), this.top + 5, TextFormat.DARK_GRAY.getColor());
    ...
    public void drawMouseoverTooltip(int mouseX, int mouseY) {
        ...
        if (mouseX >= energyDisplayX && mouseX <= energyDisplayX + ENERGY_WIDTH && mouseY >= energyDisplayY && mouseY <= energyDisplayY + ENERGY_HEIGHT) {
            ...
            toolTipLines.add(new TranslatableTextComponent("ui.galacticraft-rewoven.machine.status", ((BasicSolarPanelBlockEntity) world.getBlockEntity(blockPos)).status.toString()).setStyle(new Style().setColor(TextFormat.GRAY)).getFormattedText());
            if (((BasicSolarPanelBlockEntity) world.getBlockEntity(blockPos)).status == BasicSolarPanelStatus.COLLECTING) {
                if (world.getTimeOfDay() > 6000) {
                    toolTipLines.add(new TranslatableTextComponent("ui.galacticraft-rewoven.machine.gj_per_t", (int) ((6000D - ((double) world.getTimeOfDay() - 6000D)) / 133.3333333333D)).setStyle(new Style().setColor(TextFormat.LIGHT_PURPLE)).getFormattedText());
                } else {
                    toolTipLines.add(new TranslatableTextComponent("ui.galacticraft-rewoven.machine.gj_per_t", (int) (((double) world.getTimeOfDay()) / 133.3333333333D)).setStyle(new Style().setColor(TextFormat.LIGHT_PURPLE)).getFormattedText());
                }
            }
            toolTipLines.add("\u00A76" + new TranslatableTextComponent("ui.galacticraft-rewoven.machine.current_energy", new GalacticraftEnergyType().getDisplayAmount(((BasicSolarPanelBlockEntity) world.getBlockEntity(blockPos)).getEnergy().getCurrentEnergy()).setStyle(new Style().setColor(TextFormat.BLUE))).getFormattedText() + "\u00A7r");
            toolTipLines.add("\u00A7c" + new TranslatableTextComponent("ui.galacticraft-rewoven.machine.max_energy", new GalacticraftEnergyType().getDisplayAmount(((BasicSolarPanelBlockEntity) world.getBlockEntity(blockPos)).getEnergy().getMaxEnergy())).getFormattedText() + "\u00A7r");
